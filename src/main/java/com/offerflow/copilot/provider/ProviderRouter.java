...
public class ProviderRouter {
    ...
    public ProviderConfigCheck configCheck() {
        ...
        return new ProviderConfigCheck(
                ...
                warnings,
                "P4B sandbox uses local-rule/no-op providers only; no real external model calls are made.");
    }
    ...
    private List<String> configWarnings() {
        ...
        if (properties.realCallEnabled()) {
            baseWarnings.add("realCallEnabled is true, but P4B adapters remain no-op and do not call external networks.");
        }
        if (properties.rawResponseSave()) {
            baseWarnings.add("rawResponseSave is true, but P4B ProviderResponse still forces rawResponseSaved=false.");
        }
