...
P3A 使用 H2 in-memory 数据库作为本地 demo/test persistence。应用启动时执行 `schema.sql` 建表，并由 `PersistenceSeedService` 在空表中插入脱敏 seed demo 数据。重复调用 seed 不会重复插入已有数据。
...
- `human_review_item`：人工复核队列、风险词、证据引用、人工备注和状态。
...
使用 MySQL 前需要先提供独立迁移策略、备份策略、脱敏策略和删除策略。
